...
This terraform code deploys a single-region Secured Virtual WAN (Vwan) topology.
...
We can see that the endpoint is a public IP address, ***20.105.216.1***. We can see the CNAME `vwan23-spoke3-09b1-app.privatelink.azurewebsites.net` created for the app service which recursively resolves to the public IP address.
...
6.1 Recall the hostname of the app service in ***spoke3*** as done in Step 5.2. In our example, the hostname is `vwan23-spoke3-09b1-app.azurewebsites.net`.
