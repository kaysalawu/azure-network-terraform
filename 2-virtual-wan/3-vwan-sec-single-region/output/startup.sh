...
echo  "\$(timeout 3 curl -4 -kL --max-time 3.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null https://vwan23spoke3sa4dd9.blob.core.windows.net/spoke3/spoke3.txt) - https://vwan23spoke3sa4dd9.blob.core.windows.net/spoke3/spoke3.txt"
EOF
...
echo  "\$(timeout 3 curl -6 -kL --max-time 3.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null https://vwan23spoke3sa4dd9.blob.core.windows.net/spoke3/spoke3.txt) - https://vwan23spoke3sa4dd9.blob.core.windows.net/spoke3/spoke3.txt"
EOF
