...
conn Tunnel0
    left=10.10.1.9
    leftid=52.138.169.132
    right=4.209.224.53
    rightid=4.209.224.53
    auto=start
    mark=100
...
10.10.1.9 4.209.224.53 : PSK "changeme"
10.10.1.9 4.209.224.153 : PSK "changeme"
...
echo  "\$(curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null https://vwan23spoke3sa2bc7.blob.core.windows.net/spoke3/spoke3.txt) - https://vwan23spoke3sa2bc7.blob.core.windows.net/spoke3/spoke3.txt"
EOF
