				...
				<TLSExtensions
					...
					<FilteringInfo
						...
						<CAHashList Enabled="true">
							<IssuerHash>A4 D1 A0 AD 53 54 1B 99 1F B2 64 14 36 E2 89 96 75 0E 2F 65 </IssuerHash>
...
$Connection = Get-VpnConnection -Name Vwan24-vwan_Vwan24-vhub1-hub
if($connection -ne $null)
{
	try
    {
     if ($Force -eq $false) {
	    ...
	    Remove-VpnConnection -Name Vwan24-vwan_Vwan24-vhub1-hub -Force -ErrorAction Stop
...
try
{
    ...
    {
        Add-VpnConnection -Name Vwan24-vwan_Vwan24-vhub1-hub -ServerAddress hub0.5x6w19hufewneqh47on36x0q.vpn.azure.com -TunnelType Ikev2 -AuthenticationMethod Eap -SplitTunneling:$True -RememberCredential -EncryptionLevel Optional -EapConfigXmlStream $EAP -PassThru
    } else {
        Add-VpnConnection -Name Vwan24-vwan_Vwan24-vhub1-hub -ServerAddress hub0.5x6w19hufewneqh47on36x0q.vpn.azure.com -TunnelType Ikev2 -AuthenticationMethod MachineCertificate -SplitTunneling:$True -RememberCredential -EncryptionLevel Optional -PassThru
    }
}
catch
{
...
Add-VpnConnectionRoute -ConnectionName Vwan24-vwan_Vwan24-vhub1-hub -DestinationPrefix 192.168.11.0/24
Add-VpnConnectionRoute -ConnectionName Vwan24-vwan_Vwan24-vhub1-hub -DestinationPrefix 10.11.0.0/20
Add-VpnConnectionRoute -ConnectionName Vwan24-vwan_Vwan24-vhub1-hub -DestinationPrefix 10.11.16.0/20
Add-VpnConnectionRoute -ConnectionName Vwan24-vwan_Vwan24-vhub1-hub -DestinationPrefix 10.1.0.0/20
Add-VpnConnectionRoute -ConnectionName Vwan24-vwan_Vwan24-vhub1-hub -DestinationPrefix 192.168.0.0/24
