...
echo  "\$(timeout 4 curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null vwan24-spoke3-8dce-app.azurewebsites.net) - vwan24-spoke3-8dce-app.azurewebsites.net"
echo  "\$(timeout 4 curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null vwan24-spoke6-8dce-app.azurewebsites.net) - vwan24-spoke6-8dce-app.azurewebsites.net"
EOF
...
nping -c 3 --tcp -p 80 vwan24-spoke3-8dce-app.azurewebsites.net > /dev/null 2>&1
nping -c 3 --tcp -p 80 vwan24-spoke6-8dce-app.azurewebsites.net > /dev/null 2>&1
EOF
...
while [ \$i -lt 4 ]; do
    ...
    ab -n \$1 -c \$2 vwan24-spoke3-8dce-app.azurewebsites.net > /dev/null 2>&1
    ab -n \$1 -c \$2 vwan24-spoke6-8dce-app.azurewebsites.net > /dev/null 2>&1
    let i=i+1
  sleep 2
done
EOF
