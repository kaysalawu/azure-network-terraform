...
line vty
!
ip route 0.0.0.0/0 10.22.12.1
ip route 192.168.22.69/32 10.22.12.1
ip route 192.168.22.68/32 10.22.12.1
ip route 10.5.0.0/16 10.22.12.1
!
...
echo  "\$(curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null spoke6.p.hub2.az.corp) - spoke6.p.hub2.az.corp"
