...
<img src="../../images/scenarios/2-1-vwan-single-region.png" alt="Virtual WAN - Single Region" width="900">
...
Login to virtual machine `Vwan21-spoke1-vm` via the [serial console](https://learn.microsoft.com/en-us/troubleshoot/azure/virtual-machines/serial-console-overview#access-serial-console-for-virtual-machines-via-azure-portal):
...
```sh
vwan21spoke3sa07c5.blob.core.windows.net
```
...
We can see that the endpoint is a public IP address, ***20.60.145.4***. We can see the CNAME `vwan21spoke3sa07c5.privatelink.blob.core.windows.net.` created for the storage account which recursively resolves to the public IP address.
...
**6.1** Login to on-premises virtual machine `Vwan21-branch1-vm` via the [serial console](https://learn.microsoft.com/en-us/troubleshoot/azure/virtual-machines/serial-console-overview#access-serial-console-for-virtual-machines-via-azure-portal):
  - username = ***azureuser***
  - password = ***Password123***

 We will test access from `Vwan21-branch1-vm` to the storage account for ***spoke3*** via the private endpoint in ***hub1***.
...
vwan21spoke3sa07c5.blob.core.windows.net        canonical name = vwan21spoke3sa07c5.privatelink.blob.core.windows.net.
Name:   vwan21spoke3sa07c5.privatelink.blob.core.windows.net
Address: 10.11.7.99
```

We can see that the storage account hostname resolves to the private endpoint ***10.11.7.99*** in ***hub1***. The following is a summary of the DNS resolution from `Vwan21-branch1-vm`:

- On-premises server `Vwan21-branch1-vm` makes a DNS request for `vwan21spoke3sa07c5.blob.core.windows.net`
- The request is received by on-premises DNS server `Vwan21-branch1-dns`
- The DNS server resolves `vwan21spoke3sa07c5.blob.core.windows.net` to the CNAME `vwan21spoke3sa07c5.privatelink.blob.core.windows.net`
...
**8.1** Login to on-premises virtual machine `Vwan21-branch1-nva` via the [serial console](https://learn.microsoft.com/en-us/troubleshoot/azure/virtual-machines/serial-console-overview#access-serial-console-for-virtual-machines-via-azure-portal):
  - username = ***azureuser***
  - password = ***Password123***
