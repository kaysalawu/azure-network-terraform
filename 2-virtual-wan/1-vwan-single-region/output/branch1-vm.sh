...
az login --identity -u /subscriptions/b120edff-2b3e-4896-adb7-55d2918f337f/resourceGroups/Vwan21RG/providers/Microsoft.ManagedIdentity/userAssignedIdentities/Vwan21-user
...
echo  "\$(timeout 5 curl -kL --max-time 5.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null https://vwan21spoke3sa35a4.blob.core.windows.net/spoke3/spoke3.txt) - https://vwan21spoke3sa35a4.blob.core.windows.net/spoke3/spoke3.txt"
EOF
...
nping -c 10 --tcp -p 80 https://vwan21spoke3sa35a4.blob.core.windows.net/spoke3/spoke3.txt > /dev/null 2>&1
EOF
...
while [ \$i -lt 8 ]; do
    ...
    ab -n \$1 -c \$2 https://vwan21spoke3sa35a4.blob.core.windows.net/spoke3/spoke3.txt > /dev/null 2>&1
    let i=i+1
  sleep 5
done
EOF
