...
6\. (Optional) Login to all the spoke VMs and run the `ping-dns` script to test DNS resolution to all destinations. Compare your results to the expected result in the file [ping-dns-expected.txt](../tests/ping-dns-expected.txt).
