...
[← Previous](./3.%20DNS%20Resolution%20between%20Spokes.md) | [Next →](./5.%20High%20Availability%20(HA).md)
...
⚙️ 1\. Verify that the VNet of `Vwan_Dns-branch1-vm` is configured to use the on-premises custom DNS server.
...
⚙️ 2\. Login to the on-premises server `Vwan_Dns-branch1-vm` via the [serial console](https://learn.microsoft.com/en-us/troubleshoot/azure/virtual-machines/serial-console-overview#access-serial-console-for-virtual-machines-via-azure-portal):
* username = ***azureuser***
* password = ***Password123***

You should now be in a shell terminal session `azureuser@vm:~$` where we will run the following test.

⚙️ 3\. Run a DNS query to resolve the on-premises DNS ***vm.spoke1.eu.az.corp***.
...
We are unable to resolve the DNS record for ***vm.spoke1.eu.az.corp*** from on-premises.
...
⚙️ 2\. View the DNS configuration file `/etc/unbound/unbound.conf.d/forward.conf`.
...
⚙️ 5\. Run a DNS query to resolve the on-premises DNS ***vm.spoke1.eu.az.corp***.
...
⚙️ 1\. Log back into the on-premises server `Vwan_Dns-branch1-vm` via the serial console

⚙️ 2\. Run a DNS query to resolve the on-premises DNS ***vm.spoke1.eu.az.corp***.
...
⚙️ 3\. Run a DNS query to resolve the on-premises DNS ***vm.spoke3.us.az.corp***.
...
We cannot resolve the DNS record for ***vm.spoke3.us.az.corp*** from on-premises.
...
We will fix this issue in the next exercise.
