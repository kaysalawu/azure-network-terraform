...
We will now run a DNS query for the on-premises DNS name ***vm.branch1.corp*** from the spoke1 VM `Vwan_Dns-spoke1-vm1`. The on-premises [Unbound](https://en.wikipedia.org/wiki/Unbound_(DNS_server)) DNS server is the authoritative DNS server for the ***.corp*** domain.
...
From ***spoke1*** virtual machine `Vwan_Dns-spoke1-vm1`, we can now resolve on-premises DNS names - ***vm.branch1.corp*** and ***vm.branch2.corp***. We still cannot resolve DNS names in the other spokes. This is expected as we do not have any configuration to allow spoke-to-spoke DNS resolution.
