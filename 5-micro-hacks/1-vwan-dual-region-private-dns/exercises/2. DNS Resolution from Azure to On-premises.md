...
```sh
command_in="az network vnet subnet show -g $RG_NAME --name "DnsResolverInboundSubnet" --query id -o tsv"
command_out="az network vnet subnet show -g $RG_NAME --name "DnsResolverOutboundSubnet" --query id -o tsv"
SHARED1_DNS_IN_SUBNET_ID=$(eval $command_in --vnet-name $SHARED1_VNET_NAME)
SHARED2_DNS_IN_SUBNET_ID=$(eval $command_in --vnet-name $SHARED2_VNET_NAME)
SHARED1_DNS_OUT_SUBNET_ID=$(eval $command_out --vnet-name $SHARED1_VNET_NAME)
SHARED2_DNS_OUT_SUBNET_ID=$(eval $command_out --vnet-name $SHARED2_VNET_NAME)
```
...
1\. Create a DNS forwarding rule for the on-premises DNS domain ***.corp*** for `shared1-dns-forwarding-ruleset`.
...
2\. Create a DNS forwarding rule for the on-premises DNS domain ***.corp*** for `shared2-dns-forwarding-ruleset`.
...
![mh51-2-2-azure-to-onprem-test.png](../../images/../../images/microhacks/mh51/mh51-2-2-azure-to-onprem-test.png)
...
vm.branch1.corp - 10.10.0.5 -OK 4.176 ms
vm.spoke1.eu.az.corp - 10.1.0.5 -OK 0.043 ms
vm.spoke2.eu.az.corp -  -NA
vm.branch2.corp - 10.20.0.5 -OK 88.924 ms
vm.spoke3.eu.az.corp -  -NA
vm.spoke4.us.az.corp -  -NA
icanhazip.com - 104.18.114.97 -NA
```

From ***spoke1*** virtual machine `Vwan_Dns-spoke1-vm1`, we can now resolve on-premises DNS names.

## Task 7 (Optional): Test DNS Resolution from all Spokes

Login to the other spoke virtual machines and run the `ping-dns` test script for DNS resolution.

## Review!

You have learned how to configure the private DNS resolver and DNS forwarding rules to enable DNS resolution from Azure to on-premises.
