...
1\. Create VNet links with auto-registration from each private DNS zone to its corresponding spoke VNet.
...
1. In this scenario, the VM uses the default (Azure-provided) DNS servers for all DNS resolution.
...
1\. Verify that the VNet of `Vwan_Dns-spoke1-vm` has no custom DNS servers configured; which confirms that the VNet will use the Azure DNS servers for all DNS resolution.
...
vm.branch1.corp -  -NA
vm.spoke1.eu.az.corp - 10.1.0.5 -OK 0.042 ms
