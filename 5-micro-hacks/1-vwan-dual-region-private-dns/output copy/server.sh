...
echo "branch2 - 10.30.0.5 -\$(timeout 3 ping -qc2 -W1 10.30.0.5 2>&1 | awk -F'/' 'END{ print (/^rtt/? "OK "\$5" ms":"NA") }')"
...
echo "vm.spoke3.eu.az.corp - \$(timeout 3 dig +short vm.spoke3.eu.az.corp | tail -n1) -\$(timeout 3 ping -qc2 -W1 vm.spoke3.eu.az.corp 2>&1 | awk -F'/' 'END{ print (/^rtt/? "OK "\$5" ms":"NA") }')"
...
echo  "\$(timeout 4 curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null 10.30.0.5) - branch2 (10.30.0.5)"
...
echo  "\$(timeout 4 curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null spoke3-c345.azurewebsites.net) - spoke3-c345.azurewebsites.net"
echo  "\$(timeout 4 curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null spoke6-c345.azurewebsites.net) - spoke6-c345.azurewebsites.net"
EOF
