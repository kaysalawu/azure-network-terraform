...
conn Tunnel0
    left=10.10.1.9
    leftid=52.236.36.155
    right=40.127.248.65
    rightid=40.127.248.65
    auto=start
    mark=100
...
10.10.1.9 40.127.248.65 : PSK "changeme"
10.10.1.9 40.127.248.66 : PSK "changeme"
...
echo  "\$(curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null https://ne31spoke3sa1830.blob.core.windows.net/spoke3/spoke3.txt) - https://ne31spoke3sa1830.blob.core.windows.net/spoke3/spoke3.txt"
EOF
