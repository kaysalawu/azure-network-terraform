...
conn Tunnel0
    left=10.30.1.9
    leftid=52.186.18.101
    right=4.157.232.236
    rightid=4.157.232.236
    auto=start
    mark=100
...
10.30.1.9 4.157.232.236 : PSK "changeme"
10.30.1.9 4.157.236.46 : PSK "changeme"
10.30.1.9 52.169.13.28 : PSK "changeme"
...
ip route 10.22.16.4/32 vti0
ip route 10.22.16.5/32 vti1
...
echo  "\$(curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null https://ne32spoke3sa72e9.blob.core.windows.net/spoke3/spoke3.txt) - https://ne32spoke3sa72e9.blob.core.windows.net/spoke3/spoke3.txt"
echo  "\$(curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null https://ne32spoke6sa72e9.blob.core.windows.net/spoke6/spoke6.txt) - https://ne32spoke6sa72e9.blob.core.windows.net/spoke6/spoke6.txt"
EOF
