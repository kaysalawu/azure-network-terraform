...
conn Tunnel0
    left=10.30.1.9
    leftid=52.168.54.150
    right=172.210.98.165
    rightid=172.210.98.165
    auto=start
    mark=100
...
conn Tunnel1
    left=10.30.1.9
    leftid=52.168.54.150
    right=172.210.98.173
    rightid=172.210.98.173
    auto=start
    mark=200
    leftupdown="/etc/ipsec.d/ipsec-vti.sh"
conn Tunnel2
...
10.30.1.9 172.210.98.165 : PSK "changeme"
10.30.1.9 172.210.98.173 : PSK "changeme"
10.30.1.9 52.138.210.29 : PSK "changeme"
...
case "$PLUTO_CONNECTION" in
  Tunnel0)
    ...
    VTI_REMOTEADDR=10.22.16.5
    ;;
  Tunnel1)
    VTI_INTERFACE=vti1
    VTI_LOCALADDR=10.10.10.5
    VTI_REMOTEADDR=10.22.16.4
    ;;
  Tunnel2)
    VTI_INTERFACE=vti2
...
ip route 10.22.16.5/32 vti0
ip route 10.22.16.4/32 vti1
...
neighbor 10.22.16.5 remote-as 65515
neighbor 10.22.16.5 ebgp-multihop 255
neighbor 10.22.16.5 update-source lo
neighbor 10.22.16.4 remote-as 65515
neighbor 10.22.16.4 ebgp-multihop 255
neighbor 10.22.16.4 update-source lo
...
address-family ipv4 unicast
  ...
  neighbor 10.22.16.5 soft-reconfiguration inbound
  neighbor 10.22.16.4 soft-reconfiguration inbound
...
echo  "\$(curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null https://hs14spoke3sa074e.blob.core.windows.net/spoke3/spoke3.txt) - https://hs14spoke3sa074e.blob.core.windows.net/spoke3/spoke3.txt"
echo  "\$(curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null https://hs14spoke6sa074e.blob.core.windows.net/spoke6/spoke6.txt) - https://hs14spoke6sa074e.blob.core.windows.net/spoke6/spoke6.txt"
EOF
