...
echo  "$(timeout 3 curl -4 -kL --max-time 3.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null https://hs11spoke3saf97e.blob.core.windows.net/spoke3/spoke3.txt) - https://hs11spoke3saf97e.blob.core.windows.net/spoke3/spoke3.txt"
EOF
...
echo  "$(timeout 3 curl -6 -kL --max-time 3.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null https://hs11spoke3saf97e.blob.core.windows.net/spoke3/spoke3.txt) - https://hs11spoke3saf97e.blob.core.windows.net/spoke3/spoke3.txt"
EOF
...
nping -c 5 --tcp-connect -p 80,8080 https://hs11spoke3saf97e.blob.core.windows.net/spoke3/spoke3.txt > /dev/null 2>&1
EOF
...
while [ $i -lt 5 ]; do
    ...
    ab -n $1 -c $2 https://hs11spoke3saf97e.blob.core.windows.net/spoke3/spoke3.txt > /dev/null 2>&1
    let i=i+1
  sleep 5
done
EOF
...
nping -c 5 -6 --tcp-connect -p 80,8080 https://hs11spoke3saf97e.blob.core.windows.net/spoke3/spoke3.txt > /dev/null 2>&1
EOF
...
while [ $i -lt 8 ]; do
  ...
  if [ $? -ne 0 ]; then
    ...
    ab -s 3 -n $1 -c $2 [$(get_ipv6 https://hs11spoke3saf97e.blob.core.windows.net/spoke3/spoke3.txt)]/ > /dev/null 2>&1
  # check if ab command was successful
  if [ $? -ne 0 ]; then
    echo "target: https://hs11spoke3saf97e.blob.core.windows.net/spoke3/spoke3.txt failed"
    exit 1
  else
    echo "target: https://hs11spoke3saf97e.blob.core.windows.net/spoke3/spoke3.txt passed"
  fi
    let i=i+1
  sleep 5
done
