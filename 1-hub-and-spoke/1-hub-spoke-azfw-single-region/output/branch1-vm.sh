...
az login --identity -u /subscriptions/b120edff-2b3e-4896-adb7-55d2918f337f/resourceGroups/Hs11RG/providers/Microsoft.ManagedIdentity/userAssignedIdentities/Hs11-user
...
echo  "\$(timeout 4 curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null hs11-spoke3-b86c.azurewebsites.net) - hs11-spoke3-b86c.azurewebsites.net"
EOF
...
nping -c 10 --tcp -p 80 hs11-spoke3-b86c.azurewebsites.net > /dev/null 2>&1
EOF
...
while [ \$i -lt 8 ]; do
    ...
    ab -n \$1 -c \$2 hs11-spoke3-b86c.azurewebsites.net > /dev/null 2>&1
    let i=i+1
  sleep 5
done
EOF
