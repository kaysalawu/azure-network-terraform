...
echo  "\$(timeout 3 curl -kL --max-time 3.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null https://hs13spoke3sa47ba.blob.core.windows.net/spoke3/spoke3.txt) - https://hs13spoke3sa47ba.blob.core.windows.net/spoke3/spoke3.txt"
EOF
...
nping -c 10 --tcp -p 80 https://hs13spoke3sa47ba.blob.core.windows.net/spoke3/spoke3.txt > /dev/null 2>&1
EOF
...
while [ \$i -lt 8 ]; do
    ...
    ab -n \$1 -c \$2 https://hs13spoke3sa47ba.blob.core.windows.net/spoke3/spoke3.txt > /dev/null 2>&1
    let i=i+1
  sleep 5
done
EOF
