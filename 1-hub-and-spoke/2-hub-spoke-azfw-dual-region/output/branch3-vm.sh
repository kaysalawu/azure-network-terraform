...
echo  "\$(timeout 4 curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null hs12-spoke3-8f46-app.azurewebsites.net) - hs12-spoke3-8f46-app.azurewebsites.net"
echo  "\$(timeout 4 curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null hs12-spoke6-8f46-app.azurewebsites.net) - hs12-spoke6-8f46-app.azurewebsites.net"
EOF
...
nping -c 3 --tcp -p 80 hs12-spoke3-8f46-app.azurewebsites.net > /dev/null 2>&1
nping -c 3 --tcp -p 80 hs12-spoke6-8f46-app.azurewebsites.net > /dev/null 2>&1
EOF
...
while [ \$i -lt 4 ]; do
    ...
    ab -n \$1 -c \$2 hs12-spoke3-8f46-app.azurewebsites.net > /dev/null 2>&1
    ab -n \$1 -c \$2 hs12-spoke6-8f46-app.azurewebsites.net > /dev/null 2>&1
    let i=i+1
  sleep 2
done
EOF
