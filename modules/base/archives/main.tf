...
resource "local_file" "settings_opnsense" {
  count    = var.config_nva.enable && var.config_nva.type == "opnsense" ? 1 : 0
  filename = "${path.module}/templates/output.tpl"
...
locals {
  JSON_GLOBAL = {
    ...
    publicIPAddress               = "8.8.8.8" #length(azurerm_public_ip.opnsense) > 0 ? azurerm_public_ip.opnsense[0].ip_address : ""
    opnSenseSecondarytrustedNicIP = var.config_nva.scenario_option == "Active-Active" ? "SOME" : ""
  }
  JSON_IPSEC = {
    Phase1RemoteGw1               = try(module.s2s_vpngw[0].public_ip0, "8.8.8.8")
    Phase1RemoteGw2               = try(module.s2s_vpngw[0].public_ip1, "8.8.8.8")
  ...
  settings_opnsense = jsonencode({
    fileUris = [
      "${local.JSON_GLOBAL.OpnScriptURI}${local.JSON_GLOBAL.ShellScriptName}"
    ],
    commandToExecute = "sh ${local.JSON_GLOBAL.ShellScriptName} '${jsonencode(local.JSON_GLOBAL)}' '${jsonencode(local.JSON_IPSEC)}'"
  })
}
