...
nping -c 3 --tcp -p 80 vm.hub1.eu.az.corp > /dev/null 2>&1
EOF
...
cat <<EOF > /usr/local/bin/iperf3-server
#! /bin/bash
iperf3 -s -p 8080 -i 60
EOF
chmod a+x /usr/local/bin/iperf3-server

# iperf3 client

cat <<EOF > /usr/local/bin/iperf3-client
#! /bin/bash
if [ -z "\$1" ]; then
  COUNT=3
else
...
iperf3 -c 10.11.0.5 -t 900 -P \$COUNT -l 8192 -p 8080 -i 60
EOF
chmod a+x /usr/local/bin/iperf3-client
