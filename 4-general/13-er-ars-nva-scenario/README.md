...
200 (0.040476s) - 10.11.7.99 - https://lab13spoke3sa8181.blob.core.windows.net/spoke3/spoke3.txt
```
...
000 (0.008114s) -  - https://lab13spoke3sa8181.blob.core.windows.net/spoke3/spoke3.txt
```
...
* lab13spoke3sa\<AAAA\>.blob.core.windows.net
...
```sh
spoke3_storage_account=$(az storage account list -g Lab13_Er_Ars_Nva_Scenario_RG --query "[?contains(name, 'lab13spoke3sa')].name" -o tsv)
...
```sh
lab13spoke3sa8181.blob.core.windows.net
```
...
lab13spoke3sa8181.blob.core.windows.net  canonical name = lab13spoke3sa8181.privatelink.blob.core.windows.net.
lab13spoke3sa8181.privatelink.blob.core.windows.net      canonical name = blob.db3prdstr19a.store.core.windows.net.
...
We can see that the endpoint is a public IP address, ***20.150.75.36***. We can see the CNAME `lab13spoke3sa8181.privatelink.blob.core.windows.net.` created for the storage account which recursively resolves to the public IP address.
...
**6.1** Login to on-premises virtual machine `Lab13-branch1Vm` via the [serial console](https://learn.microsoft.com/en-us/troubleshoot/azure/virtual-machines/serial-console-overview#access-serial-console-for-virtual-machines-via-azure-portal):
  - username = ***azureuser***
  - password = ***Password123***

 We will test access from `Lab13-branch1Vm` to the storage account for ***spoke3*** via the private endpoint in ***hub1***.
...
Non-authoritative answer:
lab13spoke3sa8181.blob.core.windows.net  canonical name = lab13spoke3sa8181.privatelink.blob.core.windows.net.
Name:   lab13spoke3sa8181.privatelink.blob.core.windows.net
Address: 10.11.7.99
```
...
We can see that the storage account hostname resolves to the private endpoint ***10.11.7.99*** in ***hub1***. The following is a summary of the DNS resolution from `Lab13-branch1Vm`:

- On-premises server `Lab13-branch1Vm` makes a DNS request for `lab13spoke3sa8181.blob.core.windows.net`
- The request is received by on-premises DNS server `Lab13-branch1-dns`
- The DNS server resolves `lab13spoke3sa8181.blob.core.windows.net` to the CNAME `lab13spoke3sa8181.privatelink.blob.core.windows.net`
...
Whilst still logged into the on-premises server `Lab13-branch1Vm` via the serial console, we will test connectivity to all virtual machines using the `trace-ipv4` script using the linux `tracepath` utility.
...
**8.1** Login to on-premises virtual machine `Lab13-branch1Nva` via the [serial console](https://learn.microsoft.com/en-us/troubleshoot/azure/virtual-machines/serial-console-overview#access-serial-console-for-virtual-machines-via-azure-portal):
  - username = ***azureuser***
  - password = ***Password123***
...
```sh
bash ../../scripts/_cleanup.sh Lab13_Er_Ars_Nva_Scenario_RG
```
...
```sh
3-hub-spoke-nva-single-region$    bash ../../scripts/_cleanup.sh Lab13_Er_Ars_Nva_Scenario_RG

Resource group: Lab13_Er_Ars_Nva_Scenario_RG

⏳ Checking for diagnostic settings on resources in Lab13_Er_Ars_Nva_Scenario_RG ...
➜  Checking firewall ...
➜  Checking vnet gateway ...
    ❌ Deleting: diag setting [Lab13-hub1-vpngw-diag] for vnet gateway [Lab13-hub1-vpngw] ...
➜  Checking vpn gateway ...
➜  Checking er gateway ...
➜  Checking app gateway ...
⏳ Checking for azure policies in Lab13_Er_Ars_Nva_Scenario_RG ...
Done!
```
...
```sh
az group delete -g Lab13_Er_Ars_Nva_Scenario_RG --no-wait
```
