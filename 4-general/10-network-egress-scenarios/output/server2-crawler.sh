...
function check_service_endpoints() {
  ...
  service_endpoints=$(timeout 10 az network vnet subnet show -g $RESOURCE_GROUP --vnet-name G10-hub-vnet --name "ProductionSubnet" --query "serviceEndpoints[].service" -o tsv 2>/dev/null)
...
function check_private_subnet() {
  ...
  #####################################################
  echo -e "   Subnet --> ProductionSubnet"
  default_outbound_access=$(timeout 10 az network vnet subnet list --resource-group $RESOURCE_GROUP --vnet-name G10-hub-vnet --query "[?name=='ProductionSubnet'].defaultOutboundAccess" -o tsv 2>/dev/null)
...
function check_management_access() {
  ...
  #####################################################
  host=$(echo https://management.azure.com/subscriptions?api-version=2020-01-01 | awk -F/ '{print $3}')
...
function download_blob() {
  ...
  echo -e "\n6. Blob (Data Plane)"
  #####################################################
  host=$(echo https://g10hub33bc.blob.core.windows.net/storage/storage.txt | awk -F/ '{print $3}')
  echo -e "   url = https://g10hub33bc.blob.core.windows.net/storage/storage.txt"
  echo -e "   host = $host"
  resolve_dns $host 2>/dev/null
  get_azure_service_tag_from_host $host 2>/dev/null

  echo "   Retrieving blob content ..."
  blob_access=$(timeout 10 az storage blob download --account-name g10hub33bc -c storage -n storage.txt --query content -o tsv 2>/dev/null)
...
function access_keyvault_secret() {
  ...
  #####################################################
  host=$(echo https://g10-hub-kv33bc.vault.azure.net/secrets/message | awk -F/ '{print $3}')
  echo -e "   url: https://g10-hub-kv33bc.vault.azure.net/secrets/message"
  echo -e "   host: $host"
  resolve_dns $host
  get_azure_service_tag_from_host $host 2>/dev/null

  echo "   Accessing secret ..."
  secret_value=$(timeout 10 az keyvault secret show --vault-name g10-hub-kv33bc --name message --query value -o tsv 2>/dev/null)
