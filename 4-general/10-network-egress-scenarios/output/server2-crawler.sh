...
function check_service_endpoints() {
  ...
  if ! service_endpoints=$(timeout 5 az network vnet subnet show -g G10_NetworkEgress_RG --vnet-name G10-hub-vnet --name "ProductionSubnet" --query "serviceEndpoints[].service" -o tsv 2>/dev/null); then
...
function check_private_subnet() {
  ...
  #####################################################
  echo -e "   Subnet --> ProductionSubnet"
  if ! default_outbound_access=$(timeout 5 az network vnet subnet list --resource-group G10_NetworkEgress_RG --vnet-name G10-hub-vnet --query "[?name=='ProductionSubnet'].defaultOutboundAccess" -o tsv 2>/dev/null); then
...
function download_blob() {
  ...
  echo -e "\n6. Blob (Data Plane)"
  #####################################################
  local host=$(echo https://g10huba221.blob.core.windows.net/storage/storage.txt | awk -F/ '{print $3}')
  echo -e "   url = https://g10huba221.blob.core.windows.net/storage/storage.txt"
  echo -e "   host = $host"
  resolve_dns $host 2>/dev/null
  get_azure_service_tag_from_host $host 2>/dev/null

  echo -e "   az storage account keys list -g G10_NetworkEgress_RG --account-name g10huba221"
  if STORAGE_ACCOUNT_KEY=$(timeout 5 az storage account keys list -g G10_NetworkEgress_RG --account-name g10huba221 --query "[0].value" -o tsv 2>/dev/null); then
    echo "   az storage blob download --account-name g10huba221 -c storage -n storage.txt --account-key <KEY>"
    blob_access=$(timeout 5 az storage blob download --account-name g10huba221 -c storage -n storage.txt --account-key $STORAGE_ACCOUNT_KEY --query content -o tsv 2>/dev/null)
  else
    ...
    if ! STORAGE_ACCESS_TOKEN=$(timeout 5 curl -H Metadata:true "http://169.254.169.254:80/metadata/identity/oauth2/token?resource=https%3A%2F%2Fstorage.azure.com&api-version=2018-02-01" -s | jq -r .access_token); then
      ...
      echo "   curl https://g10huba221.blob.core.windows.net/storage/storage.txt ..."
      blob_access=$(timeout 5 curl -s -H "x-ms-version: 2019-02-02" -H "Authorization: Bearer $STORAGE_ACCESS_TOKEN" "https://g10huba221.blob.core.windows.net/storage/storage.txt")
    fi
  fi
...
function access_keyvault_secret() {
  ...
  #####################################################
  local host=$(echo https://g10-hub-kva221.vault.azure.net/secrets/message | awk -F/ '{print $3}')
  echo -e "   url: https://g10-hub-kva221.vault.azure.net/secrets/message"
  echo -e "   host: $host"
  resolve_dns $host
  get_azure_service_tag_from_host $host 2>/dev/null

  echo "   az keyvault secret show --vault-name g10-hub-kva221 --name message"
  if ! secret_value=$(timeout 5 az keyvault secret show --vault-name g10-hub-kva221 --name message --query value -o tsv 2>/dev/null); then
    ...
    if ! VAULT_ACCESS_TOKEN=$(timeout 5 curl -H Metadata:true "http://169.254.169.254/metadata/identity/oauth2/token?resource=https%3A%2F%2Fvault.azure.net&api-version=2018-02-01" -s | jq -r .access_token); then
      ...
      echo "curl https://g10-hub-kva221.vault.azure.net/secrets/message?api-version=7.2"
      secret_value=$(timeout 5 curl -H "Authorization : Bearer $VAULT_ACCESS_TOKEN" "https://g10-hub-kva221.vault.azure.net/secrets/message?api-version=7.2" -o secret.txt 2>/dev/null)
    fi
  fi
