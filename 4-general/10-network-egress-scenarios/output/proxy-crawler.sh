...
function check_private_subnet() {
  ...
  #####################################################
  echo -e "   Subnet --> PublicSubnet"
  default_outbound_access=$(timeout 5 az network vnet subnet list --resource-group G10_NetworkEgress_RG --vnet-name G10-hub-vnet --query "[?name=='PublicSubnet'].defaultOutboundAccess" -o tsv 2>/dev/null)
