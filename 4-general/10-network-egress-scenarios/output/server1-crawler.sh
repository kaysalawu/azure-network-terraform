...
function download_blob() {
  ...
  echo -e "\n6. Blob (Data Plane)"
  #####################################################
  local host=$(echo https://lab10hub77ed.blob.core.windows.net/storage/storage.txt | awk -F/ '{print $3}')
  echo -e "   url = https://lab10hub77ed.blob.core.windows.net/storage/storage.txt"
  echo -e "   host = $host"
  resolve_dns $host 2>/dev/null
  get_azure_service_tag_from_host $host 2>/dev/null
  storage_account_key=""
  ...
  blob_access=""
  echo -e "   az storage account keys list -g Lab10_NetworkEgress_RG --account-name lab10hub77ed"
  if storage_account_key=$(timeout 5 az storage account keys list -g Lab10_NetworkEgress_RG --account-name lab10hub77ed --query "[0].value" -o tsv 2>/dev/null); then
    echo "   az storage blob download --account-name lab10hub77ed -c storage -n storage.txt --account-key <KEY>"
    blob_access=$(timeout 5 az storage blob download --account-name lab10hub77ed -c storage -n storage.txt --account-key $storage_account_key --query content -o tsv 2>/dev/null)
  else
    ...
    if ! storage_access_token=$(timeout 5 curl -H Metadata:true "http://169.254.169.254:80/metadata/identity/oauth2/token?resource=https%3A%2F%2Fstorage.azure.com&api-version=2018-02-01" -s | jq -r .access_token); then
      ...
      echo "   curl https://lab10hub77ed.blob.core.windows.net/storage/storage.txt ..."
      blob_access=$(timeout 5 curl -s -H "Cache-Control: no-cache" -H "Pragma: no-cache" -H "x-ms-version: 2019-02-02" -H "Authorization: Bearer $storage_access_token" "https://lab10hub77ed.blob.core.windows.net/storage/storage.txt")
    fi
  fi
...
function access_keyvault_secret() {
  ...
  #####################################################
  local host=$(echo https://lab10-hub-kv77ed.vault.azure.net/secrets/message | awk -F/ '{print $3}')
  echo -e "   url: https://lab10-hub-kv77ed.vault.azure.net/secrets/message"
  echo -e "   host: $host"
  resolve_dns $host
  get_azure_service_tag_from_host $host 2>/dev/null
  secret_value=""
  vault_access_token=""
  echo "   az keyvault secret show --vault-name lab10-hub-kv77ed --name message"
  if ! secret_value=$(timeout 5 az keyvault secret show --vault-name lab10-hub-kv77ed --name message --query value -o tsv 2>/dev/null); then
    ...
    if ! vault_access_token=$(timeout 5 curl -H Metadata:true "http://169.254.169.254/metadata/identity/oauth2/token?resource=https%3A%2F%2Fvault.azure.net&api-version=2018-02-01" -s | jq -r .access_token); then
      ...
      echo "curl https://lab10-hub-kv77ed.vault.azure.net/secrets/message?api-version=7.2"
      secret_value=$(timeout 5 curl -H "Cache-Control: no-cache" -H "Pragma: no-cache" -H "Authorization : Bearer $vault_access_token" "https://lab10-hub-kv77ed.vault.azure.net/secrets/message?api-version=7.2" -o secret.txt 2>/dev/null)
    fi
  fi
