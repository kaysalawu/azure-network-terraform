...
function check_private_subnet() {
  ...
  #####################################################
  echo -e "   Subnet --> ProductionSubnet"
  default_outbound_access=$(timeout 5 az network vnet subnet list --resource-group G10_NetworkEgress_RG --vnet-name G10-hub-vnet --query "[?name=='ProductionSubnet'].defaultOutboundAccess" -o tsv 2>/dev/null)
...
function download_blob() {
  ...
  echo -e "   az storage account keys list -g G10_NetworkEgress_RG --account-name g10hub99a2"
  if STORAGE_ACCOUNT_KEY=$(timeout 5 az storage account keys list -g G10_NetworkEgress_RG --account-name g10hub99a2 --query "[0].value" -o tsv 2>/dev/null); then
    echo "   az storage blob download --account-name g10hub99a2 -c storage -n storage.txt --account-key <KEY>"
    blob_access=$(timeout 5 az storage blob download --account-name g10hub99a2 -c storage -n storage.txt --account-key $STORAGE_ACCOUNT_KEY --query content -o tsv 2>/dev/null)
  else
    ...
    if ! STORAGE_ACCESS_TOKEN=$(timeout 5 curl -H Metadata:true "http://169.254.169.254:80/metadata/identity/oauth2/token?resource=https%3A%2F%2Fstorage.azure.com&api-version=2018-02-01" -s | jq -r .access_token); then
      ...
      echo "   curl https://g10hub99a2.blob.core.windows.net/storage/storage.txt ..."
      blob_access=$(timeout 5 curl -s -H "x-ms-version: 2019-02-02" -H "Authorization: Bearer $STORAGE_ACCESS_TOKEN" "https://g10hub99a2.blob.core.windows.net/storage/storage.txt")
    fi
  fi
...
function access_keyvault_secret() {
  ...
  echo "   az keyvault secret show --vault-name g10-hub-kv99a2 --name message"
  if ! secret_value=$(timeout 5 az keyvault secret show --vault-name g10-hub-kv99a2 --name message --query value -o tsv 2>/dev/null); then
    ...
    if ! VAULT_ACCESS_TOKEN=$(timeout 5 curl -H Metadata:true "http://169.254.169.254/metadata/identity/oauth2/token?resource=https%3A%2F%2Fvault.azure.net&api-version=2018-02-01" -s | jq -r .access_token); then
      ...
      echo "curl https://g10-hub-kv99a2.vault.azure.net/secrets/message?api-version=7.2"
      secret_value=$(timeout 5 curl -H "Authorization : Bearer $VAULT_ACCESS_TOKEN" "https://g10-hub-kv99a2.vault.azure.net/secrets/message?api-version=7.2" -o secret.txt 2>/dev/null)
    fi
  fi
