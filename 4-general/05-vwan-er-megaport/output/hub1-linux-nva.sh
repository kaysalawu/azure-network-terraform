...
ip route 192.168.22.69/32 10.11.2.1
ip route 192.168.22.68/32 10.11.2.1
!
!-----------------------------------------
! Route Maps
!-----------------------------------------
...
echo "spoke3vm.eu.az.corp - \$(dig +short spoke3vm.eu.az.corp | tail -n1) -\$(ping -qc2 -W1 spoke3vm.eu.az.corp 2>&1 | awk -F'/' 'END{ print (/^rtt/? "OK "\$5" ms":"NA") }')"
...
echo  "\$(curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null spoke3vm.eu.az.corp) - spoke3vm.eu.az.corp"
