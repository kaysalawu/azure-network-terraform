...
echo  "\$(timeout 4 curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null g10-spoke3-d45e.azurewebsites.net) - g10-spoke3-d45e.azurewebsites.net"
EOF
...
nping -c 10 --tcp -p 80 g10-spoke3-d45e.azurewebsites.net > /dev/null 2>&1
EOF
...
while [ \$i -lt 8 ]; do
    ...
    ab -n \$1 -c \$2 g10-spoke3-d45e.azurewebsites.net > /dev/null 2>&1
    let i=i+1
  sleep 5
done
EOF
