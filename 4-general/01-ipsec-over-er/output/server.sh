...
echo  "\$(timeout 3 curl -kL --max-time 3.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null https://lab01spoke3saf69f.blob.core.windows.net/spoke3/spoke3.txt) - https://lab01spoke3saf69f.blob.core.windows.net/spoke3/spoke3.txt"
EOF
