...
We can see that we are ECMP load balancing traffic across two MSEE paths - **172.16.0.4/30** (for `Lab08-hub1Vm`)  and **172.16.0.0/30** (for `Lab08-spoke1Vm`). Trace path shows asymmetrical routing for both paths. This is the normal behaviour for traffic over ExpressRoute circuits - as inbound traffic goes via the ExpressRoute gateway and outbound traffic goes directly to the ExpressRoute circuit on the MSEE.
...
```bash
08-network-reliability$ bash ../../scripts/vnet-gateway/get_bgp_peer_status.sh Lab08_Network_SRE_RG
...
```bash
08-network-reliability$ bash ../../scripts/vnet-gateway/get_route_tables.sh Lab08_Network_SRE_RG
...
```bash
08-network-reliability$ bash ../../scripts/vnet-gateway/get_er_route_tables.sh Lab08_Network_SRE_RG
...
The network virtual appliance (NVA) is the next hop for all traffic to destination `10.0.0.0/8`; which includes our test vitual machines - `Lab08-hub1Vm1` and `Lab08-spoke1Vm`. Once th etraffic reaches the NVA, it will follow the BGP route towards ExpressRoute because it has a higher local preference than BGP routes received over the IPsec tunnels. The NVA will forward the traffic through its default gateway (on the untrusted network) interface (NIC) towards the virtual network (VNet).
...
10.1.0.0/20       eBGP        172.16.0.2      Lab08-er1-pri     12076
...
10.11.0.0/20      eBGP        172.16.0.2      Lab08-er1-pri     12076
...
10.11.16.0/20     eBGP        172.16.0.2      Lab08-er1-pri     12076
