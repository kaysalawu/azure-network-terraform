...
## 2. ExpressRoute Circuit ER1 All Down <!-- omit from toc -->

[← 1. Previous](./1.%20ER1-primary-down.md) ✤ [ Next → ](./3.%20ER2-primary-down.md)

Contents
- [Overview](#overview)
- [1. ExpressRoute Circuit `Poc08-er1` Completely Down](#1-expressroute-circuit-poc08-er1-completely-down)
...
In this section, we'll shut down the entire ExpressRoute circuit `Poc08-er1` on the Megaport Coud Router (MCR) leaving only the second ExpressRoute circuit `Poc08-er2` active. The diagram shows the connectivity paths from `P08-Branch2Vm` to both `P08-Hub1Vm` and `P08-Spoke1Vm`. Traffic is now only flows through Microsoft Enterprise Edge (MSEE) links of `Poce08-er2`.

<img src="../images/scenarios/2-er1-all-down.png" alt="er1-sec-enabled" width="1100">

The dashboard marker shows that ExpressRoute circuit `Poc08-er1` is completely second circuit `Poc08-er2` is up. The ExpressRoute gateway `Poc08-hub1-ergw` is zone-redundant and has two instances of the gateway in different availability zones. The dashboard shows active traffic flow across the two instances.
...
## 1. ExpressRoute Circuit `Poc08-er1` Completely Down
...
-------------------------------------
 ...
 1:  10.20.1.9                                             0.843ms
 1:  10.20.1.9                                             1.221ms
 2:  10.20.16.4                                            6.233ms asymm  3
 3:  172.16.0.17                                          13.045ms asymm  4
 4:  172.16.0.14                                          22.418ms asymm  5
 5:  no reply
 6:  10.11.0.5                                            24.790ms reached
...
-------------------------------------
 1?: [LOCALHOST]                      pmtu 1500
 1:  10.20.1.9                                             2.209ms
 2:  10.20.16.4                                            3.352ms asymm  3
 3:  172.16.0.21                                          12.691ms asymm  4
 4:  172.16.0.14                                          22.963ms asymm  5
 5:  no reply
 6:  10.1.0.5                                             25.840ms reached
     Resume: pmtu 1500 hops 6 back 6
...
We can see that we are now using the MSEE path **172.16.0.12/30** to reach `Poc08-hub1Vm` and`Poc08-spoke1Vm`. Trace path shows asymmetrical routing for both paths. This is the normal behaviour for traffic over ExpressRoute circuits - as inbound traffic goes via the ExpressRoute gateway and outbound traffic goes directly to the ExpressRoute circuit on the MSEE.
...
-------------------------------------
 1?: [LOCALHOST]                      pmtu 1500
 1:  10.11.16.7                                            1.749ms asymm  2
 1:  10.11.16.6                                            1.721ms asymm  2
 2:  172.16.0.13                                          12.001ms asymm  3
 3:  172.16.0.18                                          22.063ms asymm  4
 4:  no reply
 5:  10.20.1.9                                            24.761ms
 6:  10.20.0.5                                            25.252ms reached
     Resume: pmtu 1500 hops 6 back 6

hub1
-------------------------------------
 1?: [LOCALHOST]                      pmtu 1500
 1:  10.11.0.5                                             2.102ms reached
 1:  10.11.0.5                                             1.515ms reached
...
`Poc08-spoke1Vm` uses `Poc08-er2` for the return path to `Poc08-branch2Vm`. We can see the trace path runs through the interface **172.16.0.13** which is linked to `Poc08-er1`.
...
Neighbor     ASN    LocalAddress    RoutesReceived    State
-----------  -----  --------------  ----------------  ---------
10.11.16.6   12076  10.11.16.12     8                 Connected
10.11.16.7   12076  10.11.16.12     8                 Connected
10.11.16.8   12076  10.11.16.12     0                 Connected
10.11.16.9   12076  10.11.16.12     0                 Connected
10.11.16.14  65515  10.11.16.12     1                 Connected
...
All BGP routes through `Poc08-er1` are now withdrawn from the routing table. So the on-premises network (Megaport MCR) no longer learns the Azure prefixes **10.11.0.0/20** and **10.1.0.0/20** through circuit `Poc08-er1`.
...
S   0.0.0.0/0 [1/0] via 10.20.1.1, eth0, 2d11h09m
K>* 0.0.0.0/0 [0/100] via 10.20.1.1, eth0, src 10.20.1.9, 2d11h09m
B>  10.1.0.0/20 [20/0] via 10.20.17.4 (recursive), 00:31:44
  *                      via 10.20.1.1, eth0, 00:31:44
                       via 10.20.17.5 (recursive), 00:31:44
                         via 10.20.1.1, eth0, 00:31:44
B>  10.11.0.0/20 [20/0] via 10.20.17.4 (recursive), 00:31:44
  *                       via 10.20.1.1, eth0, 00:31:44
                        via 10.20.17.5 (recursive), 00:31:44
                          via 10.20.1.1, eth0, 00:31:44
B>  10.11.16.0/20 [20/0] via 10.20.17.4 (recursive), 00:31:44
  *                        via 10.20.1.1, eth0, 00:31:44
                         via 10.20.17.5 (recursive), 00:31:44
                           via 10.20.1.1, eth0, 00:31:44
S   10.11.16.14/32 [1/0] is directly connected, vti0, 00:54:51
C>* 10.11.16.14/32 is directly connected, vti0, 00:54:51
S   10.11.16.15/32 [1/0] is directly connected, vti1, 00:47:54
C>* 10.11.16.15/32 is directly connected, vti1, 00:47:54
S>* 10.20.0.0/24 [1/0] via 10.20.1.1, eth0, 2d11h09m
C>* 10.20.1.0/24 is directly connected, eth0, 2d11h09m
C>* 10.20.2.0/24 is directly connected, eth1, 2d11h09m
S>* 10.20.17.4/32 [1/0] via 10.20.1.1, eth0, 2d11h09m
S>* 10.20.17.5/32 [1/0] via 10.20.1.1, eth0, 2d11h09m
K>* 168.63.129.16/32 [0/100] via 10.20.1.1, eth0, src 10.20.1.9, 2d11h09m
K>* 169.254.169.254/32 [0/100] via 10.20.1.1, eth0, src 10.20.1.9, 2d11h09m
C>* 192.168.20.20/32 is directly connected, lo, 2d11h09m
```
