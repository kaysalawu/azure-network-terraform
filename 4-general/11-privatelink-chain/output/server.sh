...
echo  "\$(timeout 3 curl -kL --max-time 3.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null hub1pls.eu.az.corp) - hub1pls.eu.az.corp"
echo  "\$(timeout 3 curl -kL --max-time 3.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null spoke1pls.eu.az.corp) - spoke1pls.eu.az.corp"
...
echo  "\$(timeout 3 curl -kL --max-time 3.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null https://hs14spoke3sa8ba5.blob.core.windows.net/spoke3/spoke3.txt) - https://hs14spoke3sa8ba5.blob.core.windows.net/spoke3/spoke3.txt"
EOF
