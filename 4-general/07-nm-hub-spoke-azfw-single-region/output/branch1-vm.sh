...
echo  "\$(timeout 4 curl -kL --max-time 2.0 -H 'Cache-Control: no-cache' -w "%{http_code} (%{time_total}s) - %{remote_ip}" -s -o /dev/null ne31-spoke3-13b2.azurewebsites.net) - ne31-spoke3-13b2.azurewebsites.net"
EOF
...
nping -c 3 --tcp -p 22 vm.spoke2.we.az.corp > /dev/null 2>&1
nping -c 3 --tcp -p 3389 vm.spoke2.we.az.corp > /dev/null 2>&1
nping -c 3 --tcp -p 80 ne31-spoke3-13b2.azurewebsites.net > /dev/null 2>&1
EOF
...
while [ \$i -lt 4 ]; do
    ...
    ab -n \$1 -c \$2 vm.spoke2.we.az.corp > /dev/null 2>&1
    ab -n \$1 -c \$2 vm.spoke2.we.az.corp > /dev/null 2>&1
    ab -n \$1 -c \$2 ne31-spoke3-13b2.azurewebsites.net > /dev/null 2>&1
    let i=i+1
  sleep 2
done
EOF
