...
resource "megaport_azure_connection" "azure_vcx_hub1_er2" {
  vxc_name   = "${local.megaport_prefix}-azure-vcx-hub1-er2"
  rate_limit = 50
  a_end {
...
resource "azurerm_virtual_network_gateway_connection" "azure_vcx_hub1_er2" {
  resource_group_name        = azurerm_resource_group.rg.name
  name                       = "${local.hub1_prefix}-azure-vcx-hub1-er2"
  location                   = local.hub1_location
  type                       = "ExpressRoute"
  virtual_network_gateway_id = module.hub1.ergw.id
...
resource "azurerm_virtual_network_gateway_connection" "azure_vcx_branch2_er2" {
  resource_group_name        = azurerm_resource_group.rg.name
  name                       = "${local.branch2_prefix}-azure-vcx-branch2-er2"
  location                   = local.branch2_location
  type                       = "ExpressRoute"
  virtual_network_gateway_id = module.branch2.ergw.id
