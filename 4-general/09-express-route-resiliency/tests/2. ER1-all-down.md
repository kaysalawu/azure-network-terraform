...
[← Previous](./1.%20ER1-primary-down.md) ✤ [ Next → ](./3.%20ER2-primary-down.md)

Contents
- [Overview](#overview)
- [1. ExpressRoute Circuit `Lab09-er1` Completely Down](#1-expressroute-circuit-lab09-er1-completely-down)
...
In this section, we'll shut down the entire ExpressRoute circuit `Lab09-er1` on the Megaport Coud Router (MCR) leaving only the second ExpressRoute circuit `Lab09-er2` active. The diagram shows the connectivity paths from `P08-Branch2Vm` to both `P08-Hub1Vm` and `P08-Spoke1Vm`. Traffic is now only flows through Microsoft Enterprise Edge (MSEE) links of `Poce08-er2`.

<img src="../images/scenarios/2-er1-all-down.png" alt="er1-sec-enabled" width="450">

The dashboard marker shows that ExpressRoute circuit `Lab09-er1` is completely second circuit `Lab09-er2` is up. The ExpressRoute gateway `Lab09-hub1-ergw` is zone-redundant and has two instances of the gateway in different availability zones. The dashboard shows active traffic flow across the two instances.
...
## 1. ExpressRoute Circuit `Lab09-er1` Completely Down
...
branch2
-------------------------------------
 1:  branch2Vm                                             0.079ms reached
     Resume: pmtu 65535 hops 1 back 1

hub1
-------------------------------------
 1?: [LOCALHOST]                      pmtu 1500
 1:  10.20.16.4                                            2.295ms asymm  2
 1:  10.20.16.5                                            3.791ms asymm  2
 2:  172.16.0.21                                          13.359ms asymm  3
 3:  172.16.0.14                                          25.644ms asymm  4
 4:  no reply
 5:  10.11.0.5                                            27.099ms reached
     Resume: pmtu 1500 hops 5 back 5

spoke1
-------------------------------------
 1?: [LOCALHOST]                      pmtu 1500
 1:  10.20.16.5                                            2.526ms asymm  2
 1:  10.20.16.4                                            2.431ms asymm  2
 2:  172.16.0.17                                          12.713ms asymm  3
 3:  172.16.0.14                                          24.339ms asymm  4
 4:  no reply
 5:  10.1.0.5                                             29.730ms reached
     Resume: pmtu 1500 hops 5 back 5
...
We can see that we are now using the MSEE path **172.16.0.12/30** to reach `Lab09-hub1Vm` and`Lab09-spoke1Vm`. Trace path shows asymmetrical routing for both paths. This is the normal behaviour for traffic over ExpressRoute circuits - as inbound traffic goes via the ExpressRoute gateway and outbound traffic goes directly to the ExpressRoute circuit on the MSEE.
...
-------------------------------------
 1?: [LOCALHOST]                      pmtu 1500
 1:  10.11.16.9                                            1.975ms asymm  2
 1:  10.11.16.9                                            1.532ms asymm  2
 2:  172.16.0.13                                          13.369ms asymm  3
 3:  172.16.0.22                                          24.140ms asymm  4
 4:  no reply
 5:  10.20.0.5                                            26.277ms reached
     Resume: pmtu 1500 hops 5 back 5

hub1
-------------------------------------
 1?: [LOCALHOST]                      pmtu 1500
 1:  10.11.0.5                                             2.363ms reached
 1:  10.11.0.5                                             0.954ms reached
     Resume: pmtu 1500 hops 1 back 1

spoke1
-------------------------------------
 1:  spoke1vm.internal.cloudapp.net                        0.066ms reached
     Resume: pmtu 65535 hops 1 back 1
...
`Lab09-spoke1Vm` uses `Lab09-er2` for the return path to `Lab09-branch2Vm`. We can see the trace path runs through the interface **172.16.0.13** which is linked to `Lab09-er1`.
...
All BGP routes through `Lab09-er1` are now withdrawn from the routing table. So the on-premises network (Megaport MCR) no longer learns the Azure prefixes **10.11.0.0/20** and **10.1.0.0/20** through circuit `Lab09-er1`.
