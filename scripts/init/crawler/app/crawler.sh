...
function check_private_subnet() {
  ...
  #####################################################
  echo -e "   Subnet --> ${SUBNET_NAME}"
  default_outbound_access=$(timeout 5 az network vnet subnet list --resource-group ${RESOURCE_GROUP} --vnet-name ${VNET_NAME} --query "[?name=='${SUBNET_NAME}'].defaultOutboundAccess" -o tsv 2>/dev/null)
